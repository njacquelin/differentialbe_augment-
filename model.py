...
class EquiMod(nn.Module):
    def __init__(self):
        super().__init__(cifar=False)
        self.n_head = 6
        self.n_layers = 6
        self.h_dim = 1024
        self.d_model = 384 # must be divisible by n_head
        self.d_input_augment_vector = 17

        input_size = 32 if cifar else 224
        kernel = 2 if cifar else 16
        ...
        self.conv_input = nn.Sequential(
            nn.Conv2d(3, self.h_dim, kernel_size=kernel, stride=kernel, padding=0),
        ...
        self.deconv_output = nn.Sequential(
            nn.ConvTranspose2d(self.d_model, self.h_dim, kernel_size=kernel, padding=0),
            nn.BatchNorm2d(self.h_dim, affine=False),
        )
    ...
    def forward(self, x, params):
        ...
        """
        transformation_embedding = self.transform_encoder(params)
        
        feature_map = self.conv_input(x) # (B, D, H, W)
        sequence = feature_map.view(-1, self.d_model, self.seq_size) # (B, D, S)
        sequence = torch.swapaxes(sequence, (2, 0, 1)) # (S, B, D)

        img_embedding = self.transformer_decoder(sequence, transformation_embedding) # (S, B, D)

        out_feature_map = torch.swapaxes(out_feature_map, (1, 2, 0)) # (B, D, S)
        out_feature_map = img_embedding.view(-1, self.d_model, self.map_size, self.map_size) # (B, D, H, W)
